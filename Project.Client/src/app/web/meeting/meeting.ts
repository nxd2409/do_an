...
import { DocumentType, FileType } from '../../shared/statics/document-type.static';
import { MeetingVoteAnswer, MeetingVoteStatus } from '../../shared/statics/meeting-vote-status.static';
...
export class Meeting implements OnInit, OnDestroy {
  ...
  @ViewChild('excalidrawIframe') excalidrawIframe!: ElementRef<HTMLIFrameElement>;
  ...
  individualControlStates: Map<string, { isAudioMuted: boolean; isVideoMuted: boolean }> = new Map();
  isTablet = false;

  //Speech Recognition
  isVoiceRecognitionActive = false;
  transcriptionText = '';
  private speechInitialized = false;
  currentSubtitle = '';
  subtitleUsername = '';
  private subtitleTimeout: any;
  ...
  private readonly STREAM_CONFIG = { maxAttempts: 5, baseDelay: 3000, initDelay: 500 };
  ...
  // noti tab
  tabNotifications = new Set<number>();
  ...
  private toastTimeout: Map<string, any> = new Map();
  ...
  constructor(
    ...
    this.signalR.startConnection(this.accountInfo.userName, () => this.intoTheMeeting());
  ...
  ngOnInit(): void {
    if (PlatformUtils.isMobile() && !this.isTablet) {
      this.router.navigate([`/m/meeting/${this.meetingId}`]);
      return;
    }

    this._service.getInfoMeeting(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (!res) {
  ...
  this.speechService.initialize()
    .then(() => {
      ...
      // Subscribe to transcription updates (partial results)
      this.speechService.transcription$
        ...
        .subscribe((text: string) => {
          ...
          if (this.isVoiceRecognitionActive) {
            this.broadcastTranscription(text, false);
          }
        });

      // Subscribe to final results
      this.speechService.finalResult$
        .pipe(takeUntil(this.destroy$))
        .subscribe((text: string) => {
          this.transcriptionText = text;
          ...
          if (this.isVoiceRecognitionActive) {
            this.broadcastTranscription(text, true);
          }
        });
    })
    .catch((error: Error) => {
      this.message.warning('Trình duyệt không hỗ trợ nhận dạng giọng nói. Vui lòng sử dụng Chrome hoặc Edge.');
    });
}
  ...
  private setupMessageListener() {
    this.messageListener = (event: MessageEvent) => {
      ...
      const handlers: { [key: string]: () => void } = {
        'JITSI_SCRIPT_LOADED': () => { this.jitsiScriptLoaded = true; this.initJitsi(); },
        'JITSI_SCRIPT_ERROR': () => this.message.error('Không thể kết nối đến hệ thống phòng họp trực tuyến. Vui lòng liên hệ quản trị viên hệ thống!'),
        ...
        'RECORDING_LINK_AVAILABLE': () => { this.recordingLink = link; this.message.success('File ghi hình đã sẵn sàng'); },
  ...
  getEndMeetingTooltip(): string {
    if (this.currentAccountInMeeting.isChuTri || this.currentAccountInMeeting.isThuKy) {
        return 'Kết thúc cuộc họp';
    }
  ...
  startMeeting() {
    this._service.startMeeting({ meetingId: this.meetingId }).pipe(takeUntil(this.destroy$)).subscribe();
  }

  startRecording() {
    this.sendJitsiCommand('START_RECORDING');
    this.recordingStatus = true;
    this._service.startRecording({ meetingId: this.meetingId }).pipe(takeUntil(this.destroy$)).subscribe();
  }

  stopRecording() {
    this.sendJitsiCommand('STOP_RECORDING');
    this.recordingStatus = false;
    this._service.endRecording({ meetingId: this.meetingId }).pipe(takeUntil(this.destroy$)).subscribe();
  }

  raiseHand() {
    this._service.raiseHand({
  ...
  intoTheMeeting() {
    this._service.intoTheMeeting(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe();
  }

  exitTheMeeting() {
    this._service.exitTheMeeting(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe();
  }

  getPersonalMeeting() {
    this._service.getPersonalMeeting(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        ...
        this.currentAccountInMeeting = res.find((x: any) => x.userName === this.accountInfo.userName);
        ...
        this.meetingVoteCalculate.sumMeetingPersonalVote = res.filter((x: any) => x.isParticipateInVoting)?.length;
  ...
  getOnlineUsers() {
    this.listPersonalOnline = [];
    this._service.getOnlineUsers().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        ...
        if (this.selectedUserChat) {
          this.selectedUserChat = this.listPersonalMeeting.find(x => x.userName === this.selectedUserChat.userName);
        }
      }
    });
  }
  ...
  viewRecording(file: any, mode: string) {
    ...
    if (this.currentSelectedFile.icon == 'img/multimedia.png' || this.currentSelectedFile.icon == 'img/audio.png') {
      this.videoUrl = null;
      setTimeout(() => {
  ...
  //#region MEETING VOTE
  closeControlAccount() { this.isVisibleControlAccount = false; }
  openControlAccount() { this.isVisibleControlAccount = true; }
  closeCreateVote() {
    if (this.sub) {
      this.sub.unsubscribe();
  ...
  openCreateVote() { this.isVisibleCreateVote = true; }

  closeQR() { this.isVisibleQR = false; }

  createVote() {
    this.bieuQuyetCreate.meetingId = this.meetingId;
    this._service.createMeetingVote(this.bieuQuyetCreate).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => this.closeCreateVote()
    });
  }

  getMeetingVote() {
    this._service.getMeetingVote(this.meetingId)
      .pipe(takeUntil(this.destroy$))
      .subscribe(res => {
        this.listMeetingVote = res;

        const voteProcessing = res.find((x: any) => x.status === this.meetingVoteStatus.DangBieuQuyet);
        this.isVotingProcessing = !!voteProcessing;

        this.meetingVoteCalculate.sumMeetingNotVote = res.filter((x: any) => x.status === this.meetingVoteStatus.ChuaBatDau).length;
        this.meetingVoteCalculate.sumMeetingVoteDone = res.filter((x: any) => x.status === this.meetingVoteStatus.KetThuc).length;
  ...
  private calculateVoteStatistics() {
    ...
    this.listMeetingVote.forEach((vote: any) => {
      ...
      vote.sumResultsPercentage = vote.sumResults / vote.sumPersonalVote * 100;
      ...
      vote.khongTanThanhPercentage = vote.khongTanThanh / vote.sumResults * 100;

      vote.khongBieuQuyet = countByAnswer(this.meetingVoteAnswer.KhongBieuQuyet);
      vote.khongBieuQuyetPercentage = vote.khongBieuQuyet / vote.sumResults * 100;
    });
  }
  ...
  uploadFileMeetingVote(e: any) {
    ...
    this._file.upload(formData).subscribe({
      next: (res: any) => {
        this.bieuQuyetCreate.files = [...this.bieuQuyetCreate.files, ...res.data];
      }
    });
  }
  ...
  startVoting(data: any) {
    if (this.isVotingProcessing) {
      this.message.warning('Đang có biểu quyết diễn ra. Vui lòng chờ biểu quyết kết thúc!');
      return;
    }
  ...
  detailVote(data: any) {
    this._service.getDetailVote(data.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
  ...
  private prepareVoteDetail(vote: any) {
    ...
    this.bieuQuyetCreate.sumPersonalVote = this.meetingVoteCalculate.sumMeetingPersonalVote;
    this.bieuQuyetCreate.sumResults = vote.results.length;
    this.bieuQuyetCreate.sumResultsPercentage =
      this.bieuQuyetCreate.sumResults / this.bieuQuyetCreate.sumPersonalVote * 100;

    const countByAnswer = (answer: number) =>
      vote.results.filter((x: any) => x.answer === answer).length;

    this.bieuQuyetCreate.tanThanh = countByAnswer(this.meetingVoteAnswer.TanThanh);
    this.bieuQuyetCreate.tanThanhPercentage =
      this.bieuQuyetCreate.tanThanh / this.bieuQuyetCreate.sumResults * 100;

    this.bieuQuyetCreate.khongTanThanh = countByAnswer(this.meetingVoteAnswer.KhongTanThanh);
    this.bieuQuyetCreate.khongTanThanhPercentage =
      this.bieuQuyetCreate.khongTanThanh / this.bieuQuyetCreate.sumResults * 100;

    this.bieuQuyetCreate.khongBieuQuyet = countByAnswer(this.meetingVoteAnswer.KhongBieuQuyet);
    this.bieuQuyetCreate.khongBieuQuyetPercentage =
      this.bieuQuyetCreate.khongBieuQuyet / this.bieuQuyetCreate.sumResults * 100;
  }

  viewFileVote(data: any, mode: string) {
    if (!data) return;

    const existingIndex = this.tabsViewFileDetailVote.findIndex(t => t.id === data.id);
    if (existingIndex !== -1) {
    ...
    const newTab = {
      ...
      idTab,
      mode,
      filePreview: this.sanitizer.bypassSecurityTrustResourceUrl(`only-office/only-office-viewer.html?t=${idTab}`)
    };
  ...
  openVoteProcess(voteId: any) {
    ...
    this._service.getDetailVote(voteId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
  ...
  startCountdown(data: any) {
    ...
    this.sub = interval(1000).subscribe(() => this.calculateRemainingTime(endTime));
  }
    ...
    if (diff <= 0) {
      ...
      if (this.isVisibleVoteProcess &&
        ...
        if (!hasAnswered) {
          ...
          this.message.warning('Hết thời gian biểu quyết. Hệ thống tự động ghi nhận "Không biểu quyết"');
        }
      }
  ...
  //#region DOCUMENTS
  getListFilesCommon() {
    this._service.getListFilesCommon(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        this.documentCommon = res.filter((x: any) => x.type === this.fileType.TaiLieu);
        this.documentAfterMeeting = res.filter((x: any) => x.type === this.fileType.TaiLieuSauHop);
        const orderType: any = {
  ...
  getListFilesPersonal() {
    this._file.getByRefrence(this.currentAccountInMeeting.refrenceFileId)
      .pipe(takeUntil(this.destroy$)).subscribe({
  ...
  moveToSharedDocument(file: any) {
    this._service.moveToSharedDocument(this.meetingId, file.id).pipe(takeUntil(this.destroy$)).subscribe();
  }
  ...
  uploadDocument(e: any) {
    ...
    const refrenceId = this.typeDocumentSelected === this.fileType.TaiLieu || this.typeDocumentSelected == this.fileType.TaiLieuSauHop
    ...
    this._file.uploadAndSaveInMeeting(formData).subscribe({
      next: (res: any) => {
        ...
        if (this.typeDocumentSelected === this.fileType.TaiLieuSauHop) {
          this.documentAfterMeeting = [...this.documentAfterMeeting, ...res.data];
        }
      }
    });
  }
  ...
  saveVoiceToText() {
    this._service.saveVoiceToText(this.currentSelectedFile).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
      }
    });
  }
  ...
  exportSummaryMeeting() {
    this._service.exportSummaryMeeting(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        this.getListFilesCommon();
      }
    })
  ...
  toggleAudioForUser(userName: string) {
    const state = this.individualControlStates.get(userName) || { isAudioMuted: false, isVideoMuted: false };
    state.isAudioMuted = !state.isAudioMuted;
    this.individualControlStates.set(userName, state);

    this._service.controlParticipant({
      meetingId: this.meetingId,
      username: userName,
      action: 'audio',
  ...
  kickUserFromMeeting(user: any) {
    this._service.kickParticipant({
      meetingId: this.meetingId,
      username: user.userName
    }).pipe(takeUntil(this.destroy$)).subscribe();
  }

  getUserControlState(userName: string): { isAudioMuted: boolean; isVideoMuted: boolean } {
    return this.individualControlStates.get(userName) || { isAudioMuted: false, isVideoMuted: false };
  }

  toggleAudio() { this.sendJitsiCommand('TOGGLE_AUDIO'); }
  toggleVideo() { this.sendJitsiCommand('TOGGLE_VIDEO'); }
  toggleScreenShare() { this.sendJitsiCommand('TOGGLE_SCREEN_SHARE'); }
  toggleChat() { this.sendJitsiCommand('TOGGLE_CHAT'); }
  toggleTileView() { this.sendJitsiCommand('TOGGLE_TILE_VIEW'); }
  muteEveryone() { this.sendJitsiCommand('MUTE_EVERYONE'); }

  private syncStateToServer(type: 'audio' | 'video', muted: boolean) {
    this._service.syncParticipantState({
      meetingId: this.meetingId,
      username: this.accountInfo.userName,
      action: type,
      value: muted
  ...
  private sendJitsiCommand(command: string, data?: any) {
    if (!this.jitsiReady) {
      this.message.warning('Không thể kết nối đến hệ thống phòng họp trực tuyến. Vui lòng liên hệ quản trị viên hệ thống!');
      return;
    }
  ...
  onClickSaveExcalidraw() {
    ...
    this.excalidrawIframe?.nativeElement.contentWindow?.postMessage({ type: 'GET_SCENE_DATA' }, '*');
  }
  ...
  private handleExcalidrawSave(data: any) {
    ...
    if (this.currentExcalidrawId) {
      const existingItem = this.excalidrawList.find(x => x.id === this.currentExcalidrawId);
      if (existingItem) {
        this.saveExcalidrawData(existingItem.fileName || existingItem.name, data);
      } else {
        this._service.getExcalidrawDetail(this.currentExcalidrawId)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (existingData) => {
              this.saveExcalidrawData(existingData.fileName || existingData.name, data);
            },
            error: () => {
              this.message.error('Không thể tải thông tin bảng trắng');
            }
          });
      }
      return;
  ...
  private saveExcalidrawData(name: string, sceneData: any) {
    const payload = {
      ...
      refrenceFileId: this.meetingInfo.refrenceFileId,
      meetingId: this.meetingId,
      name: name,
    ...
    this._service.saveExcalidraw(payload)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  loadExcalidrawList() {
    const refrenceFileId = this.meetingInfo.refrenceFileId;
    if (!refrenceFileId) {        
      this.excalidrawList = [];
      return;
    }
    this._service.getExcalidrawList(refrenceFileId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  loadSavedExcalidraw(item: any) {
    this._service.getExcalidrawDetail(item.id)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          this.currentExcalidrawId = res.id;
  ...
  deleteExcalidrawItem(item: any, event: Event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._service.deleteExcalidraw(item.id)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: () => {
              this.loadExcalidrawList();
  ...
  private handleStreamError(): void {
    ...
    this.message.error('Mất kết nối. Vui lòng nhấn "Kết nối lại" để thử lại hoặc chuyển sang chế độ xem khác.');
  }

  private removeStreamPlayer(): void {
    if (this.streamPlayer) {
      try { this.streamPlayer.remove(); } catch { }
  ...
  //#region ROOM CONTROL
  openRoomControlDevice(): void { this.isVisibleControlDevice = true; }
  closeRoomControlDevice(): void { this.isVisibleControlDevice = false; }
  toggleAC(power: boolean): void { }
  duplicateScreen(): void {  }
  extendScreen(): void {  }
  selectProjectorInput(input: string): void { this.controls.projectorInput = input; }
  formatTemperature(value: number): string { return `${value}°C`; }
  formatPercentage(value: number): string { return `${value}%`; }
    ...
    this.signalR.onNotification((data: any) => {
      ...
      const actions: { [key: string]: () => void } = {
        ...
        [SignalRAction.UploadFile]: () => {
          this.getListFilesCommon();
          this.addTabNotification(2);
        },
        [SignalRAction.IntoTheMeeting]: () => {
        this.getPersonalMeeting();
        this.addTabNotification(0); 
        },
        [SignalRAction.ExitTheMeeting]: () => {
          this.getPersonalMeeting();
          this.addTabNotification(0); 
        },
        ...
        [SignalRAction.SendMessage]: () => {
          ...
          const isCurrentUser = data.username === this.currentAccountInMeeting.userName;
          const isCommonGroup = this.selectedUserChat.isCommonGroup;

          const isPrivateMessageRelevant = !isCommonGroup &&
            (data.username === this.selectedUserChat.userName || isCurrentUser);

          if (isCommonGroup || isPrivateMessageRelevant) {
            this.getListMessage();
            this.addTabNotification(5);
          }
          if (!isCurrentUser && this.selectedIndex !== 5) {
            const messageData = {
              senderUserId: data.username,
              messageText: data.messageText,
              receiverUserId: data.receiverUserId || '',
              createDate: new Date(),
            };

            const sender = this.listPersonalMeeting.find(
              (p: any) => p.userName === data.username
            );

            this.showChatToast(messageData, sender);
          }
        },

        [SignalRAction.CreateMeetingVote]: () => {
          this.getMeetingVote()
          this.addTabNotification(3);
        },
        [SignalRAction.StartVoting]: () => {
          this.message.info(data.message);
          this.getMeetingVote();
          this.addTabNotification(3);
          if (this.currentAccountInMeeting?.isParticipateInVoting && !this.isVisibleVoteProcess) {
            setTimeout(() => {
        ...
        [SignalRAction.ToggleAllMedia]: () => {
          if (data.mediaType === 'audio') {
            ...
            this.sendJitsiCommand(data.value ? 'MUTE_AUDIO' : 'UNMUTE_AUDIO');
            this.message.warning(`Chủ trì đã ${data.value ? 'tắt' : 'bật'} micro của tất cả`);
          } else if (data.mediaType === 'video') {
            this.isVideoMuted = data.value;
            this.isVideoMutedAll = data.value
            this.sendJitsiCommand(data.value ? 'MUTE_VIDEO' : 'UNMUTE_VIDEO');
            this.message.warning(`Chủ trì đã ${data.value ? 'tắt' : 'bật'} camera của tất cả`);
          }
        },

        [SignalRAction.BroadcastTranscription]: () => {
            this.displaySubtitle(data.text, data.username, data.isFinal);
        },
      };
  ...
  getListMessage() {
    this._service.getListMessage({
      ...
      receiverUserId: this.selectedUserChat?.isCommonGroup ? '' : this.selectedUserChat?.userName,
    }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
  ...
  sendMessage(): void {
    ...
    this._service.sendMessage(newMessage).pipe(takeUntil(this.destroy$)).subscribe();
  ...
  private showChatToast(message: any, senderInfo: any) {
    if (message.senderUserId === this.currentAccountInMeeting.userName) {
      return;
    }

    if (this.selectedIndex === 5) {
      if (this.selectedUserChat && this.selectedUserChat.userName === message.senderUserId) {
        return;
      }
      const isCommonGroup = !message.receiverUserId || message.receiverUserId === '';
      if (isCommonGroup && this.selectedUserChat?.isCommonGroup) {
        return;
      }
    }
    
    const isCommonGroup = !message.receiverUserId || message.receiverUserId === '';
    if (!isCommonGroup && message.receiverUserId !== this.currentAccountInMeeting.userName) {
      return;
    }

    const sender = this.listPersonalMeeting?.find(
    (p: any) => p.userName === message.senderUserId
  ) || senderInfo;

    const senderName = sender?.fullName || message.senderUserId;

    const toast = {
      id: this.generateShortId(),
      senderName: isCommonGroup ? 'Trao đổi chung' : senderName,
      senderUsername: message.senderUserId,
      message: message.messageText,
      time: this.formatToastTime(new Date()),
      isCommonGroup: isCommonGroup,
      isClosing: false,
    };

    this.chatToasts.push(toast);

    if (this.chatToasts.length > 3) {
      const oldestToast = this.chatToasts[0];
    ...
    this.toastTimeout.set(toast.id, timeout);
  ...
  closeToast(toast: any, event?: Event) {
    if (event) {
      event.stopPropagation();
    }
    ...
    const timeout = this.toastTimeout.get(toast.id);
    if (timeout) {
    ...
    setTimeout(() => {
      this.chatToasts = this.chatToasts.filter((t) => t.id !== toast.id);
    }, 300);
  }
  ...
  onToastClick(toast: any) {
    ...
    if (toast.isCommonGroup) {
      targetUser = this.commonChatGroup;
    } else {
      targetUser = this.listPersonalMeeting.find(
        (p: any) => p.userName === toast.senderUsername
      );
    }
  ...
  private formatToastTime(date: Date): string {
  ...
  loadRoomWithLayout(roomCode: string) {
    ...
    this._service.getRoomDetail(roomCode).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res) => {
  ...
  loadMeetingLayout() {
    this._service.getMeetingLayout(this.meetingId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (layouts) => {
        ...
        if (!this.listPersonalMeeting || this.listPersonalMeeting.length === 0) {
  ...
  private applyLayoutToChairs(layouts: any[]) {
    layouts.forEach((layout: any) => {
      ...
      const person = this.listPersonalMeeting.find((p: any) =>
  ...
  getAssignedCount(): number {
    return this.roomItems.filter(item => item.type === 'chair' && item.assignedPerson).length;
  }
  ...
  getPersonTooltip(person: any): string {
    const role = person.isChuTri ? ' (Chủ trì)' : person.isThuKy ? ' (Thư ký)' : '';
  ...
  async toggleVoiceRecognition(): Promise<void> {
    if (!this.speechInitialized) {
      this.message.warning('Hệ thống nhận dạng giọng nói chưa sẵn sàng. Vui lòng sử dụng Chrome hoặc Edge.');
      return;
    }

    if (!this.jitsiReady) {
      this.message.warning('Vui lòng tham gia cuộc họp trước khi bật nhận dạng giọng nói');
      return;
    }
    ...
    if (this.isVoiceRecognitionActive) {
      ...
      try {
        ...
        this.isVoiceRecognitionActive = false;
        this.transcriptionText = '';
        this.message.info('Đã tắt nhận dạng giọng nói');
      } catch (error) {
        this.message.error('Lỗi khi tắt nhận dạng giọng nói');
      }
    } else {
      // Bật nhận dạng giọng nói
      try {
        await this.speechService.startRecognition();
        this.isVoiceRecognitionActive = true;
        this.message.success('Đã bật nhận dạng giọng nói');
      } catch (error) {
        this.message.error('Không thể bật nhận dạng giọng nói. Vui lòng kiểm tra quyền microphone.');
      }
    }
  }
  ...
  private broadcastTranscription(text: string, isFinal: boolean): void {
    ...
    this._service.broadcastTranscription({
      meetingId: this.meetingId,
      username: this.accountInfo.userName,
      text: text.trim(),
      isFinal: isFinal
  ...
  private displaySubtitle(text: string, username: string, isFinal: boolean): void {
    // Lấy thông tin người nói
    const speaker = this.listPersonalMeeting.find((p: any) => p.userName === username);
    const displayName = speaker?.fullName || username;

    this.currentSubtitle = text;
    this.subtitleUsername = displayName;
    ...
    // Nếu là final result, giữ subtitle 3 giây rồi tắt
    if (isFinal) {
  ...
  //#region NOTIFICATION TAB DOT RED
  private addTabNotification(tabIndex: number) {
    if (this.selectedIndex !== tabIndex) {
      this.tabNotifications.add(tabIndex);
    }
  }

  private clearTabNotification(tabIndex: number) {
    this.tabNotifications.delete(tabIndex);
  }

  hasTabNotification(tabIndex: number): boolean {
    return this.tabNotifications.has(tabIndex);
  }

  onTabChange(index: number) {
    this.selectedIndex = index;
    this.clearTabNotification(index);
  }
  //#endregion
